...
**Detachable warrants** — the bond and the warrants trade separately, so proceeds are **split**. Allocate to the **warrants at their fair value** (to APIC–stock warrants); the **residual** goes to the bonds. **Nondetachable** warrants get **no allocation** — 100% to the bonds.

**Q — Issue $1,000,000 par bonds with detachable warrants for $1,030,000; the warrants have a fair value of $30,000. Record issuance.**

```journal
{"desc": "Bonds issued with detachable warrants (warrants at FV, bonds residual)",
 "dr": [["Cash", 1030000]],
 "cr": [["Bonds payable", 1000000], ["APIC — stock warrants", 30000]]}
```

**Convertible bonds** — under US GAAP recorded **entirely as debt** at issuance (no separate equity split). On conversion, use the **book-value method → no gain or loss**: the bond's carrying value simply becomes the carrying value of the stock issued.

**Q — $1,000,000 convertible bonds with $40,000 unamortized premium (carrying value $1,040,000) are converted into 10,000 shares of $1-par common. Record the conversion.**

```journal
{"desc": "Convert bonds — book-value method, no gain/loss",
 "dr": [["Bonds payable", 1000000], ["Premium on bonds payable", 40000]],
 "cr": [["Common stock (par)", 10000], ["APIC — common stock", 1030000]]}
```

(If the bonds had a **discount**, you would *credit* the unamortized discount instead; APIC is always the plug.)
