...
**Q — A software bundle sells for 250,000 and includes three obligations with standalone prices: license 160,000, installation 20,000, and 3 years of tech support 90,000 (total 270,000). Allocate the 250,000 by relative standalone selling price, then record the cash receipt with the license and installation satisfied and support deferred.**
...
**Q — A $4,000,000 fixed-price construction contract runs four years; the estimated total cost climbs each year (3.0M → 3.2M → 4.2M → 4.3M, with % complete per the table), turning into an overall loss in Year 3. Compute the gross profit or loss recognized each year under (a) the over-time method and (b) the point-in-time method — applying the rule that an expected overall loss is booked in full immediately (over-time also reversing prior profit).**
...
Point-in-time (completed-contract style): CIP holds **costs only**; profit deferred to completion. Journal entries for **costs** (DR CIP / CR Cash-Payables), **billings** (DR AR / CR Progress billings), and **collections** (DR Cash / CR AR) are identical under both approaches — only the profit entry differs.

**Over-time profit entry** (the one the book previously only described) — Year 1 of the $4M contract, 50% complete, cost-to-date $1,500,000, revenue = 50% × $4,000,000 = $2,000,000, GP $500,000:

```journal
{"desc": "Recognize Year-1 over-time revenue and gross profit",
 "dr": [["Construction expense (cost incurred)", 1500000], ["CIP (gross profit)", 500000]],
 "cr": [["Revenue from long-term contract", 2000000]]}
```

**Completed-contract completion entry** — illustrative $1,000,000 contract, total cost $800,000 (all costs already sit in CIP, all billed):

```journal
{"desc": "At completion — recognize revenue and clear CIP/billings (GP = 200,000)",
 "dr": [["Progress billings", 1000000], ["Cost of construction", 800000]],
 "cr": [["Revenue from long-term contract", 1000000], ["CIP", 800000]]}
```

**Loss-contract entry** — recognize the entire expected loss immediately. Illustrative point-in-time contract, price $1,000,000, revised total cost $1,100,000 → **$100,000 expected loss**:

```journal
{"desc": "Book the full anticipated contract loss at once",
 "dr": [["Loss on construction contract", 100000]],
 "cr": [["Provision for loss on contract (or CIP)", 100000]]}
```

> [!TRAP]
> Under **over-time**, the loss year's charge is larger: it books the full expected loss **plus reverses all profit recognized in prior years** (in the $4M example, Year-3 over-time P&L = **(800,000)** = (200,000) total loss + reversal of the 600,000 recognized in Years 1–2). Credit **CIP** down to net realizable value; any writedown beyond the CIP balance goes to a **Provision for loss on contract** (a liability).
...
## 9. Licenses of Intellectual Property (exam supplement)

> [!EXAM]
> Not drilled in the lecture videos but a common ASC 606 exam point — a license grants a customer rights to the entity's IP, and **timing depends on the license type**.

| License type | What it is | Revenue timing |
|---|---|---|
| **Functional** (symbolic's opposite) | IP with **standalone functionality** — software, film, music, drug formula; value doesn't depend on the licensor's ongoing activity | **Point in time** — when the customer can use and benefit (access is granted) |
| **Symbolic** | IP whose value is tied to the licensor's **continuing support** — brand names, logos, franchises, team names | **Over time** — ratably over the license period |

> [!TRAP]
> **Sales/usage-based royalty exception.** When a license's consideration is a **royalty based on the customer's sales or usage**, the variable-consideration estimate/constraint does **not** apply. Recognize the royalty **at the later of** (a) when the subsequent sale/usage occurs, or (b) when the performance obligation is satisfied. So you book royalty revenue **as the customer actually sells/uses** — you do not estimate future royalties up front.
