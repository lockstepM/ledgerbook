...
```formula
Ending AOCI = beginning AOCI ± current-year OCI (PUFER) ± reclassification adjustments
```
...
- **Extraordinary items are retired** — all unusual and/or infrequent items now sit in **non-operating**.
- **Period costs** (selling, G&A, R&D) are expensed as incurred; **unexpired costs** are capitalized as assets and expensed later via **matching** (inventory → COGS · prepaid → expense · PP&E → depreciation · patent → amortization).

> [!TRAP]
> **Management bias:** *aggressive* = book higher revenue / lower expense (premature revenue, capitalize period costs, longer useful lives, higher salvage); *conservative* = the reverse. **Classification shifting** moves items between the operating and non-operating sections (e.g., a loss pushed to non-operating) to flatter core results.

**Discontinued operations** = a strategic shift with a major effect (disposal of a **major geographic area**, a **major equity-method investment**, or a **major line of business**); reported **below continuing operations, net of tax**. Three amounts flow in:
...
- **Contingent shares** enter EPS once **all conditions are met** — then **retroactively**, in **both** basic and diluted.
...
- **Preemptive right** (only if granted in the articles): lets a holder buy a proportionate share of any new issue to preserve their ownership %.
- **Appropriated retained earnings** = a within-equity reclassification (DR unappropriated RE / CR appropriated RE) earmarking RE as unavailable for dividends; **total RE and equity are unchanged**, and it can never absorb losses or be routed to income.
...
- **Stock rights** let existing holders buy more shares (usually below market): **memo entry only until exercised**; on exercise, priced at the **exercise price** (DR Cash / CR Common stock at par / plug APIC).
...
| **Reverse stock split** | the opposite — fewer shares, higher par/price; also a **memo entry only** |
