...
**Monetizing receivables:** **pledging** = collateral for a loan, AR stays on the books (**footnote only**). **Factoring without recourse** = a true sale; the factor bears collection risk (record a **due-from-factor** holdback and a **loss** for the fee). **Factoring with recourse** = a sale only if the obligation is estimable, control is surrendered, and no forced repurchase — otherwise a **secured borrowing**.
