...
**Transfers between categories** are always made **at fair value**, then you apply the **destination** category's rules. A reclassification must be **justified** (it should improve fair presentation) — an entity may **not** cherry-pick transfers to steer gains between net income and OCI:
...
> [!TRAP]
> **AFS "intend/required to sell" exception.** The allowance/OCI split above applies only when the entity **holds** the security. If the entity **intends to sell** the impaired AFS security, or it is **more likely than not it will be required to sell** it before recovering the amortized cost, write it down **directly to fair value through net income** — **no allowance, no OCI split** (the whole loss hits income).

> [!RULE]
> **Credit-loss allowances are reversible.** The HTM/AFS credit-loss **allowance is re-measured every period** and improvements (recoveries) run **back through net income** (a credit-loss *reversal*). This is a deliberate contrast with **equity-security impairment**, which is **never reversed** (see §6).
...
**Practicability exception (measurement alternative):** a **non-public** equity investment with no readily determinable fair value is carried at **cost − impairment ± observable price changes** — adjusted up or down for the price in an **orderly transaction for an identical or similar security of the same issuer**. (Not simply "cost less impairment.")

When an equity security carried at **FV through NI** is sold, gain = **selling price − carrying value** (the valuation account already holds prior unrealized changes; on sale, close the valuation account against the security and plug the realized gain — no OCI is involved for equity).
