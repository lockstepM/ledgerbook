...
> [!TRAP]
> **Dilutive vs. antidilutive is tested against income from *continuing operations* — the "control number" — not net income.** A security's effect is judged on continuing-ops EPS; the same treatment then carries through to net income and discontinued-ops EPS. (Single-line examples that use net income only work because there are no discontinued operations.)
